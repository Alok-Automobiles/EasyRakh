...
export default function DashboardClient() {
  ...
  const inventoryStats = currentDashboardData?.stats.inventory ?? stats.inventory;
  ...
  return (
    <div className="min-h-screen bg-transparent overflow-x-hidden">
      ...
      >
          ...
          <Card className="p-3 sm:p-4 lg:p-5 overflow-hidden">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-5">
              <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 min-w-0">
                ...
                <p className="mt-2 text-lg sm:text-xl font-bold text-gray-900 truncate">{(inventoryStats.totalItems || 0).toLocaleString('en-IN')}</p>
                <p className="text-[10px] sm:text-xs text-gray-500 truncate">{(inventoryStats.totalQuantity || 0).toLocaleString('en-IN')} units</p>
              </div>
              ...
              <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-3 min-w-0">
                ...
                <p className="mt-2 text-lg sm:text-xl font-bold text-gray-900 truncate" title={formatCurrency(inventoryStats.totalValue || 0)}>
              ...
              <div className="rounded-lg border border-red-100 bg-red-50 p-3 min-w-0">
                ...
                <p className="mt-2 text-lg sm:text-xl font-bold text-gray-900 truncate">{(inventoryStats.outOfStockItems || 0).toLocaleString('en-IN')}</p>
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 min-w-0">
                ...
                <p className="mt-2 text-lg sm:text-xl font-bold text-gray-900 truncate">{(inventoryStats.inactiveItems || 0).toLocaleString('en-IN')}</p>
              ...
              <div className="rounded-lg border border-amber-100 bg-amber-50 p-3 min-w-0">
                ...
                <p className="mt-2 text-lg sm:text-xl font-bold text-gray-900 truncate">{(inventoryStats.restockItems || 0).toLocaleString('en-IN')}</p>
                <p className="text-[10px] sm:text-xs text-gray-500 truncate">1-{inventoryStats.lowStockThreshold || 5} quantity</p>
              </div>
            </div>
          </Card>
        </motion.div>
        ...
        <motion.section
          variants={itemVariants}
          aria-labelledby="period-overview-heading"
          aria-busy={!!periodSectionLoading}
          className="space-y-3 overflow-visible sm:space-y-4 lg:space-y-6"
        >
          <div className="relative z-20 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <h2 id="period-overview-heading" className="mt-0.5 text-lg font-bold text-gray-900 sm:text-xl lg:text-2xl">
                Filtered Overview
              </h2>
              <p className="mt-1 text-[11px] text-gray-500 sm:text-xs" aria-live="polite">
                {periodSectionLoading
                  ? 'Updating sales and cash figures…'
                  : 'Sales and cash figures below update together.'}
              </p>
            </div>

            <div className="flex w-fit max-w-full rounded-lg border border-gray-200 bg-gray-100 p-0.5" ref={monthPickerRef}>
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-all sm:px-3 sm:py-2 sm:text-sm ${
                    filterMode === 'month'
                ...
                >
                  ...
                  {filterMode === 'month' && <span className="ml-0.5 shrink-0 text-[10px] text-gray-400">▼</span>}
                </button>
                  ...
                  <div className="absolute left-0 top-full z-50 mt-2 w-[220px] rounded-xl border border-gray-200 bg-white p-3 shadow-xl">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => setMonthPickerYear((year) => year + 1)}
                        className="rounded-md p-1 text-slate-600 hover:bg-slate-100 disabled:cursor-not-allowed"
                        aria-label="Next year"
                        disabled={monthPickerYear >= new Date().getFullYear()}
                      >
                        <ChevronRight className={`h-4 w-4 ${monthPickerYear >= new Date().getFullYear() ? 'opacity-30' : ''}`} />
                      </button>
                    </div>
                    <div className="grid grid-cols-3 gap-1.5">
                      {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map((monthName, index) => {
                        ...
                        const isFuture = monthPickerYear > new Date().getFullYear()
                          || (monthPickerYear === new Date().getFullYear() && index > new Date().getMonth());
                        const isCurrent = monthPickerYear === new Date().getFullYear() && index === new Date().getMonth();
              ...
              <div className="relative" ref={dateRangePickerRef}>
                <button
                  ...
                  onClick={() => {
                    ...
                    setDateRangePickerOpen((open) => !open);
                    setMonthPickerOpen(false);
                  }}
                  className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-all sm:px-3 sm:py-2 sm:text-sm ${
                ...
                >
                  ...
                  <span className="ml-0.5 shrink-0 text-[10px] text-gray-400">▼</span>
                </button>

                {dateRangePickerOpen && (
                  <div className="fixed inset-x-0 top-1/3 z-50 mx-auto w-[260px] rounded-xl border border-gray-200 bg-white p-4 shadow-xl sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-2">
                    <p className="mb-3 text-xs font-semibold text-slate-800">Select Date Range</p>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            if (value) {
                              const nextFrom = new Date(value + 'T00:00:00');
                              setTempFrom(nextFrom);
                              if (tempTo && nextFrom > tempTo) setTempTo(undefined);
                            } else {
                              setTempFrom(undefined);
                            }
                          }}
                          className="h-9 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            const value = event.target.value;
                            setTempTo(value ? new Date(value + 'T00:00:00') : undefined);
                          }}
                          className="h-9 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300"
                        />
                      </div>
        ...
        <div className="grid gap-3 sm:gap-4 lg:gap-6 lg:grid-cols-5 lg:items-stretch lg:grid-rows-[auto] min-w-0 overflow-hidden">
          <motion.div variants={itemVariants} className="lg:col-span-3 flex flex-col min-h-[320px] sm:min-h-[360px] min-w-0">
            <Card className="p-3 sm:p-4 lg:p-6 flex flex-col flex-1 min-h-0 overflow-hidden">
              <div className="mb-3 flex flex-wrap items-end justify-between gap-2 sm:mb-4 lg:mb-6">
                <div className="min-w-0 flex-1">
                  ...
                  <h3 className="mt-0.5 truncate text-base font-bold text-gray-900 sm:mt-1 sm:text-lg lg:text-xl" title={periodLabel ? `Cash Flow Overview – ${periodLabel}` : 'Cash Flow Overview'}>
                    Cash Flow Overview{periodLabel ? ` – ${periodLabel}` : ''}
                  </h3>
                </div>
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="flex items-center gap-1 text-[10px] sm:gap-1.5 sm:text-xs">
                    <div className="h-2 w-2 rounded-full bg-green-700 sm:h-2.5 sm:w-2.5" />
                    <span className="whitespace-nowrap text-gray-600">Cash In</span>
                  </div>
                  <div className="flex items-center gap-1 text-[10px] sm:gap-1.5 sm:text-xs">
                    <div className="h-2 w-2 rounded-full bg-red-700 sm:h-2.5 sm:w-2.5" />
                    <span className="whitespace-nowrap text-gray-600">Cash Out</span>
                  </div>
                </div>
              </div>
