...
export async function initializeIndexes(db: Db): Promise<void> {
  try {
    ...
    await Promise.all([
      ...
      inventory.createIndex({ userId: 1, fuzzySearchTokens: 1 }),
      inventory.createIndex({ userId: 1, stockStatus: 1, updatedAt: -1, createdAt: -1 }),
      inventory.createIndex({ userId: 1, brand: 1, stockStatus: 1, updatedAt: -1 }),
      inventory.createIndex({ userId: 1, location: 1, stockStatus: 1, updatedAt: -1 }),
      inventory.createIndex({ userId: 1, supplier: 1, stockStatus: 1, updatedAt: -1 }),
